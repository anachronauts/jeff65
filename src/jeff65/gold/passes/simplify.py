...
def unop(operator, sym, a_rhs):
    @pattern.match(
        ast.AstNode(
            "expr", {"exhaustive!": True, "00": token(operator), "01": P("rhs")}
        )
    )
...
def binop(operator, sym):
    ...
    def name_binop(self, lhs, rhs):
        return ast.AstNode(sym, {"lhs": lhs, "rhs": rhs})
...
def to_list(sym, t_node, a_element):
    @pattern.match(
        ast.AstNode(sym, {"exhaustive!": True, "00": P("car"), "01": P("cdr")})
    )
    def name_right_recursion(self, car, cdr):
        return ast.AstNode(sym, {a_element: car, "next": cdr})
...
def drop_if_one_child(sym):
    @pattern.match(ast.AstNode(sym, {"exhaustive!": True, "00": P.any_node("inner")}))
...
class Simplify:
    ...
    @pattern.match(ast.AstNode("unit", {"exhaustive!": True, "00": P("toplevels")}))
    def name_unit(self, toplevels):
        return ast.AstNode("unit", {"toplevels": toplevels})

    @pattern.match(ast.AstNode("alist", {"exhaustive!": True}))
    def remove_empty_alist(self):
        return None

    @pattern.match(ast.AstNode("alist_inner", {"exhaustive!": True, "00": P("arg")}))
    def collapse_alist_final(self, arg):
        return ast.AstNode("alist", {"arg": arg, "next": None})
    ...
    def collapse_alist(self, arg, cdr):
        return ast.AstNode("alist", {"arg": arg, "next": cdr})
    ...
    def collapse_stmt_use(self, unit_name):
        return ast.AstNode("use", attrs={"name": unit_name.text})
    ...
    def collapse_stmt_constant(self, name, ty, rhs):
        return ast.AstNode(
            "constant", attrs={"name": name.text, "type": ty, "value": rhs}
        )

    @pattern.match(ast.AstNode("storage", {"exhaustive!": True}))
    def storage_default(self):
        return None

    @pattern.match(
        ast.AstNode("storage", {"exhaustive!": True, "00": token(T.STORAGE_MUT)})
    )
    def storage_mut(self):
        return "mut"

    @pattern.match(
        ast.AstNode("storage", {"exhaustive!": True, "00": token(T.STORAGE_STASH)})
    )
    ...
    def collapse_stmt_let(self, storage, name, ty, value):
        return ast.AstNode(
            "let", {"name": name.text, "type": ty, "storage": storage, "value": value}
        )
    ...
    def collapse_stmt_assign(self, lhs, rhs):
        return ast.AstNode("set", {"lvalue": lhs, "rvalue": rhs})
    ...
    def collapse_stmt_fun(self, name, body):
        return ast.AstNode(
            "fun", {"name": name.text, "return": None, "args": None, "body": body}
        )

    @pattern.match(ast.AstNode("type_id", {"exhaustive!": True, "00": P("ty")}))
    ...
    @pattern.match(
        ast.AstNode("expr", {"exhaustive!": True, "00": token(T.NUMERIC, "n")})
    )
    def numeric(self, n):
        try:
    ...
    @pattern.match(
        ast.AstNode("expr", {"exhaustive!": True, "00": token(T.IDENTIFIER, "id")})
    )
    def identifier(self, id):
        return ast.AstNode("identifier", {"name": id.text})
    ...
    @pattern.match(
        ast.AstNode(
            "expr",
            {
                ...
                "02": ast.AstNode("member", {"exhaustive!": True, "00": P("member")}),
            },
        )
    )
    def name_member_access(self, namespace, member):
        return ast.AstNode(
            "member_access", attrs={"namespace": namespace, "member": member.text}
        )
    ...
    def name_call(self, function, alist):
        return ast.AstNode("call", attrs={"target": function, "args": alist})
    ...
    @pattern.match(
        ast.AstNode(
            "string_inner", {"00": P("string0"), "01": token(T.STRING, "string1")}
        )
    )
    ...
    @pattern.match(
        ast.AstNode(
            "string_inner",
            {"00": P("string0"), "01": token(T.STRING_ESCAPE, "string1")},
        )
    )
