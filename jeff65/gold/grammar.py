...
grammar = Grammar('start', [T.EOF], [
    ...
    Rule('expr', [T.OPERATOR_DEREF, 'expr'], prec=80),
    Rule('expr', [T.OPERATOR_MINUS, 'expr'], prec=70),
    Rule('expr', ['expr', T.OPERATOR_BITNOT, 'expr'], prec=65),
    ...
    Rule('expr', ['expr', T.OPERATOR_SHL, 'expr'], prec=40),
    Rule('expr', ['expr', T.OPERATOR_SHR, 'expr'], prec=40),
    Rule('expr', ['expr', T.OPERATOR_TIMES, 'expr'], prec=30),
    Rule('expr', ['expr', T.OPERATOR_DIVIDE, 'expr'], prec=30),
    Rule('expr', ['expr', T.OPERATOR_PLUS, 'expr'], prec=20),
    Rule('expr', ['expr', T.OPERATOR_MINUS, 'expr'], prec=20),
    Rule('expr', ['expr', T.OPERATOR_EQ, 'expr'], prec=10),
    Rule('expr', ['expr', T.OPERATOR_NE, 'expr'], prec=10),
    Rule('expr', ['expr', T.OPERATOR_LE, 'expr'], prec=10),
    Rule('expr', ['expr', T.OPERATOR_GE, 'expr'], prec=10),
    Rule('expr', ['expr', T.OPERATOR_LT, 'expr'], prec=10),
    ...
    Rule('storage', [T.STORAGE_MUT]),
    Rule('storage', [T.STORAGE_STASH]),

    Rule('range_to', ['expr', T.PUNCT_TO, 'expr']),

    Rule('type_id', [T.IDENTIFIER]),
    Rule('type_id', [
        T.OPERATOR_REF, T.BRACKET_OPEN, 'type_id', T.BRACKET_CLOSE]),
    Rule('type_id', [
        T.OPERATOR_REF, T.BRACKET_OPEN,
        'storage', 'type_id', T.BRACKET_CLOSE]),
    Rule('type_id', [T.OPERATOR_REF, 'type_id']),
    Rule('type_id', [T.OPERATOR_REF, 'storage', 'type_id']),
    Rule('type_id', [T.BRACKET_OPEN, 'type_id',
                     T.PUNCT_SEMICOLON, 'expr', T.BRACKET_CLOSE]),
    Rule('type_id', [T.BRACKET_OPEN, 'storage', 'type_id',
                     T.PUNCT_SEMICOLON, 'expr', T.BRACKET_CLOSE]),
    Rule('type_id', [T.BRACKET_OPEN, 'type_id',
                     T.PUNCT_SEMICOLON, 'range_to', T.BRACKET_CLOSE]),
    Rule('type_id', [T.BRACKET_OPEN, 'storage', 'type_id',
                     T.PUNCT_SEMICOLON, 'range_to', T.BRACKET_CLOSE]),
    ...
    Rule('stmt_for', [T.STMT_FOR, 'declaration',
                      T.PUNCT_IN, 'range_to', 'do_block']),
    Rule('stmt_for', [T.STMT_FOR, 'declaration',
                      T.PUNCT_IN, 'expr', 'do_block']),
    ...
    Rule('plist', ['declaration']),
    Rule('plist', ['plist', T.PUNCT_COMMA, 'declaration']),
    Rule('stmt_fun', [T.STMT_FUN, T.IDENTIFIER,
                      T.PAREN_OPEN, T.PAREN_CLOSE,
    ...
    Rule('stmt_assign', ['expr', T.OPERATOR_ASSIGN, 'expr'], prec=5),
    Rule('stmt_assign', ['expr', T.OPERATOR_ASSIGN_INC, 'expr'], prec=5),
    Rule('stmt_assign', ['expr', T.OPERATOR_ASSIGN_DEC, 'expr'], prec=5),

    Rule('block_stmt', ['stmt_constant']),
    Rule('block_stmt', ['stmt_for']),
    Rule('block_stmt', ['stmt_if']),
    Rule('block_stmt', ['stmt_let']),
    Rule('block_stmt', ['stmt_return']),
    Rule('block_stmt', ['stmt_while']),
    Rule('block_stmt', ['stmt_assign']),
    Rule('block_stmt', ['expr'], prec=0),
    Rule('block', []),
    Rule('block', ['block', 'block_stmt']),

    Rule('unit_stmt', ['stmt_constant']),
    Rule('unit_stmt', ['stmt_isr']),
    Rule('unit_stmt', ['stmt_let']),
    Rule('unit_stmt', ['stmt_use']),
    Rule('unit_stmt', ['stmt_fun']),
    Rule('unit', []),
    Rule('unit', ['unit', 'unit_stmt']),
...
comment_grammar = Grammar('start', [T.COMMENT_CLOSE, T.WHITESPACE, T.EOF], [
    ...
    Rule('comment_inner', ['comment_inner', T.COMMENT_TEXT],
         mode=Mode.COMMENT),
    Rule('comment_inner', ['comment_inner', 'comment_nested'],
         mode=Mode.COMMENT),
